...
namespace Decent.Minecraft.Client.Java
{
    public class JavaWorld : IWorld
    {
        ...
        public async Task<T> GetBlockAsync<T>(float x, float y, float z) where T : class, IBlock
        {
        ...
        public async Task<T> GetBlockAsync<T>(Vector3 coordinates) where T : class, IBlock
        {
            return await GetBlockAsync<T>(coordinates.X, coordinates.Y, coordinates.Z);
        }

        public async Task<IBlock> GetBlockAsync(float x, float y, float z)
        {
            return await GetBlockAsync<IBlock>(x, y, z);
        }

        public async Task<IBlock> GetBlockAsync(Vector3 coordinates)
        {
            return await GetBlockAsync<IBlock>(coordinates);
        }

        public T GetBlock<T>(float x, float y, float z) where T : class, IBlock
        {
            return GetBlockAsync<T>(x, y, z).Result;
        }

        public T GetBlock<T>(Vector3 coordinates) where T : class, IBlock
        {
        ...
        {
            return GetBlock<IBlock>(x, y, z);
        }

        public IBlock GetBlock(Vector3 coordinates)
        {
            return GetBlock<IBlock>(coordinates);
        }

        public IWorld SetBlock(IBlock block, float x, float y, float z)
        {
            SetBlockAsync(block, x, y, z).Wait();
            return this;
        }

        public IWorld SetBlock(IBlock block, Vector3 coordinates)
        {
            SetBlockAsync(block, coordinates).Wait();
            return this;
        }

        public IWorld SetBlock<TBlock>(float x, float y, float z) where TBlock : class, IBlock, new()
        {
            SetBlockAsync<TBlock>(new Vector3(x, y, z)).Wait();
            return this;
        }

        public IWorld SetBlock<TBlock>(Vector3 coordinates) where TBlock : class, IBlock, new()
        {
        ...
        public async Task<IWorld> SetBlockAsync(IBlock block, Vector3 coordinates)
        {
            await SetBlockAsync(block, coordinates.X, coordinates.Y, coordinates.Z);
            return this;
        }

        public async Task<IWorld> SetBlockAsync<TBlock>(float x, float y, float z) where TBlock : class, IBlock, new()
        {
            await SetBlockAsync<TBlock>(new Vector3(x, y, z));
            return this;
        }

        public async Task<IWorld> SetBlockAsync<TBlock>(Vector3 coordinates) where TBlock : class, IBlock, new()
        {
        ...
        public IBlock[,,] GetBlocks(Vector3 corner1, Vector3 corner2)
        {
            return GetBlocksAsync(corner1, corner2).Result;
        }

        public async Task<IBlock[,,]> GetBlocksAsync(Vector3 corner1, Vector3 corner2)
        {
            ...
            var result = new IBlock[Math.Abs(x1 - x2) + 1, Math.Abs(y1 - y2) + 1, Math.Abs(z1 - z2) + 1];
            var x = 0;
            var y = 0;
            var z = 0;
        ...
        public IWorld SetBlocks(IBlock block, Vector3 corner1, Vector3 corner2)
        {
